...
class MealPlan {
    ...
    /**
     * Get meal within date range
     */
    static async findByDateRange(userId, startDate, endDate) {
        const result = await db.query(
            `
                SELECT 
                mp.id, mp.user_id, mp.recipe_id, mp.meal_date::text as meal_date, mp.meal_type, mp.created_at, mp.updated_at, r.name as recipe_name, r.image_url, r.prep_time, r.cook_time
                FROM meal_plans mp
    ...
    static async getStats(userId) {
        const result = await db.query(
            `
                ...
                COUNT(*) 
                FILTER (WHERE meal_date > CURRENT_DATE AND meal_date < CURRENT_DATE + INTERVAL '7 days') as this_week_count
                FROM meal_plans
                WHERE user_id = $1
            `,
            [userId],
