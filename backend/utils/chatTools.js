...
const saveRecipeDeclaration = {
    name: 'save_recipe',
    description:
        "Save the recipe that was most recently generated by generate_recipe in this conversation. ONLY call this AFTER the user has explicitly confirmed they want to save the recipe (e.g., 'yes', 'save it'). Takes no arguments.",
    parameters: {
        type: 'object',
        properties: {},
    },
...
export const runTool = async (name, args, userId) => {
    ...
    if (name === 'generate_recipe') {
        ...
        // cache server-side. LLM sees only short ID and summary
        pendingByUser.set(userId, {
            recipe: normalized,
            generatedAt: Date.now(),
        })

        // return only what the LLM needs to summarize and reference
        return {
    ...
    if (name === 'save_recipe') {
        ...
        if (!pending) {
            throw new Error(
                'No recently generated recipe to save. Please generate a recipe first.',
            )
        }
        if (Date.now() - pending.generatedAt > PENDING_TTL_MS) {
            pendingByUser.delete(userId)
            throw new Error(
                'The generated recipe has expired. Please generate it again.',
            )
        }

        const saved = await Recipe.create(userId, pending.recipe)
        pendingByUser.delete(userId)

        return { id: saved.id, name: saved.name, saved: true }
    }
